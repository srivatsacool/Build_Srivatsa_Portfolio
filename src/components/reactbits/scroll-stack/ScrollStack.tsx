...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    // Cast string to HTMLElement because we are querying DOM
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      const stackDist = itemStackDistance!;
  ...
  useLayoutEffect(() => {
    const scroller = scrollerRef.current;
    if (!scroller && !useWindowScroll) return; // Wait for scroller if not window scroll
    ...
    cards.forEach((card, i) => {
      ...
      // card.style.webkitTransform = 'translateZ(0)'; // Removed to avoid typing issues mostly, or cast to any if needed
